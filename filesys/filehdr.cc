...
{
	numBytes = (fileSize > MaxFileSize) ? MaxFileSize : fileSize;
	fileSize -= numBytes;
...
{
	DEBUG(dbgFile, "1:offset: " << offset);
	// return (dataSectors[offset / SectorSize]);
	int SectorIndex = offset / SectorSize;
	if (SectorIndex < NumDirect)
	{
		DEBUG(dbgFile, "dataSectors[SectorIndex]: " << dataSectors[SectorIndex]);
		return dataSectors[SectorIndex];
	}
	else
	{
		ASSERT(nextFilehdr != NULL);
		nextFilehdr->ByteToSector(offset - MaxFileSize);
	}
}
