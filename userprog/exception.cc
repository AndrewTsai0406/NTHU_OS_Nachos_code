	...
	case SyscallException:
		...
		case SC_Create:
			...
			val = kernel->machine->ReadRegister(4);
			{
				...
				//cout << filename << endl;
				status = kernel->fileSystem->Create(filename, initialSize);
				kernel->machine->WriteRegister(2, (int)status);
			}
			kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
			kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
		...
		case SC_Write:
			ch_add = kernel->machine->ReadRegister(4);
			val = kernel->machine->ReadRegister(5);
			fileID = kernel->machine->ReadRegister(6);
			{
				char *ch_S = &(kernel->machine->mainMemory[ch_add]); // s for string
				numChar = kernel->fileSystem->WriteFile(ch_S, val, fileID);
				//numChar = 0;
			}
		...
		case SC_Read:
			...
			{
				char *ch_S = &(kernel->machine->mainMemory[ch_add]); // s for string
				numChar = kernel->fileSystem->ReadFile(ch_S, numChar, fileID);
				//numChar = 0;
			}
			kernel->machine->WriteRegister(2, (int)numChar);
